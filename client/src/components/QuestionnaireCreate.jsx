...
import './scss/QuestionnaireCreate.scss';
...
const QuestionnaireCreate = () => {
    ...
    const handleQuestionnaireSubmit = async (event) => {
        ...
        const questionnaireData = {
            questionaireId: parseInt(questionaireId),
            questionaireName,
            questions
        };
        ...
        try {
            await axios.post('http://localhost:3001/api/questionnaires', questionnaireData);
    ...
    const handleAddQuestion = () => {
        setQuestions([...questions, { 
            ...
            type: 'text', 
            options: [], 
            followUpQuestions: [], // Changed from followUpQuestion to followUpQuestions array
            questions: [] 
        }]);
    };
    ...
    const handleQuestionChange = (index, event) => {
        ...
        if (name === 'type') {
            if (['radio', 'checkbox', 'radio|text'].includes(value)) {
                const optionType = value === 'radio|text' ? 'radio|text' : value;
                newQuestions[index].options = [{ option: '', type: optionType }];
            } else {
                newQuestions[index].options = [];
            }
            newQuestions[index].followUpQuestions = []; // Reset to empty array instead of null
        }
        setQuestions(newQuestions);
    };
